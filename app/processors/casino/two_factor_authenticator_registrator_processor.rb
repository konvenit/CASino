...
class CASino::TwoFactorAuthenticatorRegistratorProcessor < CASino::Processor
  ...
  def process(user)
    ...
    if person.allow_2fa_auth?
      two_factor_authenticator = CASino::TwoFactorAuthenticator.create! user: user, secret: ROTP::Base32.random_base32, expiry: CASino::TwoFactorAuthenticator.lifetime.from_now
